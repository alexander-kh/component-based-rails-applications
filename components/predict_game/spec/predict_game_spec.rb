RSpec.describe PredictGame do
  ...
  describe "perform" do
    ...
    it "will publish a message for a successful prediction" do
      ...
      expect(@subscriber.prediction_succeeded_result).not_to be_nil
      expect(@subscriber.prediction_failed_result).to be_nil
    end
    
    it "will publish a message for an unsuccessful prediction" do
      @predict_game.perform(@team1, @team1)
      
      expect(@subscriber.prediction_succeeded_result).to be_nil
      expect(@subscriber.prediction_failed_result).not_to be_nil
    end
  end
end
